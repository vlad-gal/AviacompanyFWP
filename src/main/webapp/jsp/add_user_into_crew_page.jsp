...
<head>
    ...
    <title><fmt:message key="local.addUserIntoCrew"/></title>
</head>
    ...
    <div class="container">
        <div class="row justify-content-md-center">
            <div class="card-wrapper">
                <div class="card fat">
                    <div class="card-body">
                        <h5 class="card-title text-center"><fmt:message key="local.addUserIntoCrew"/></h5>
                        <c:if test="${errorAddUserIntoCrewFlag eq true}">
                            <h6 class="errorLoginPass text-center"><fmt:message key="local.errorAddUserIntoCrew"/></h6>
                        </c:if>
                        <c:if test="${addUserIntoCrewSuccessfulFlag eq true}">
                            <h6 class="alert-success text-center"><fmt:message key="local.addUserIntoCrewSuccessful"/></h6>
                        </c:if>
                        ...
                        <c:if test="${availablePlacesInCrewFlag eq true}">
                            <h6 class="errorLoginPass text-center"><fmt:message key="local.availablePlacesInCrew"/> ${addingUser.role.roleName} = ${availablePlacesInCrew}</h6>
                        </c:if>
                        <form method="POST" name="addUserIntoCrewForm"
                              action="${pageContext.request.contextPath}/controller">
                            <input type="hidden" name="command" value="add_user_into_crew">
                            <div class="form-group">
                                <label for="addingUser"><fmt:message key="local.addingUser"/></label>
                                <input id="addingUser" type="text" class="form-control"
                                       name="addingUser"
                                       value="${addingUser.firstName} ${addingUser.lastName} - ${addingUser.role.roleName}"
                                       disabled>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group m-0">
                                <input type="submit" class="btn btn-primary btn-block"
                                       value="<fmt:message key="local.addUser"/>">
                            </div>
                        </form>
                    </div>
                </div>
