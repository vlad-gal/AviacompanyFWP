...
<head>
    ...
    <title><fmt:message key="local.common.updateFlight"/></title>
</head>
        ...
        <div class="container">
            <div class="row justify-content-md-center">
                <div class="card-wrapper">
                    <div class="card fat">
                        <div class="card-body">
                            <h5 class="card-title text-center"><fmt:message key="local.common.updateFlight"/></h5>
                            ...
                            <form method="POST" class="login-validation" name="updateFlightForm" action="${pageContext.request.contextPath}/controller">
                                        ...
                                        <c:forEach var="airport" items="${airports}">
                                            <c:choose>
                                                <c:when test="${updatingFlight.departureAirport.id == airport.id}">
                                                    <option value="${updatingFlight.departureAirport.id}" selected>${updatingFlight.departureAirport.airportName}, ${updatingFlight.departureAirport.city}, ${updatingFlight.departureAirport.country}</option>
                                                </c:when>
                                                <c:otherwise>
                                ...
                                <div class="form-group">
                                    <label for="destinationAirport"><fmt:message key="local.flight.destinationAirport"/>*</label>
                                        ...
                                        <c:forEach var="airport" items="${airports}">
                                            <c:choose>
                                                <c:when test="${updatingFlight.destinationAirport.id == airport.id}">
                                                    <option value="${updatingFlight.destinationAirport.id}" selected>${updatingFlight.destinationAirport.airportName}, ${updatingFlight.destinationAirport.city}, ${updatingFlight.destinationAirport.country}</option>
                                                </c:when>
                                                <c:otherwise>
                                                    <option value="${airport.id}">
                                                            ${airport.airportName}, ${airport.city}, ${airport.country}
                                        ...
                                        <c:forEach var="crew" items="${crews}">
                                            <c:choose>
                                                <c:when test="${updatingFlight.crew.id == crew.id}">
                                                    <option value="${updatingFlight.crew.id}" selected>${updatingFlight.crew.crewName}</option>
                                                </c:when>
                                                <c:otherwise>
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="aircraft" name="aircraft" class="custom-select" required>
                                            ...
                                            <c:choose>
                                                <c:when test="${updatingFlight.aircraft.id == aircraft.id}">
                                                    <option value="${updatingFlight.aircraft.id}" selected>${updatingFlight.aircraft.aircraftName}, <fmt:message key="local.aircraft.tailNumber"/> - ${updatingFlight.aircraft.tailNumber}</option>
                                                </c:when>
                                                <c:otherwise>
                                    ...
                                    <div class="form-group">
                                        <label for="operator"><fmt:message key="local.common.operator"/>*</label>
                                        <select id="operator" name="operator" class="custom-select" required>
                                            <c:forEach var="operator" items="${operators}">
                                            <c:choose>
                                                    <c:when test="${updatingFlight.operator.id == operator.id}">
                                                        <option value="${updatingFlight.operator.id}" selected>${updatingFlight.operator.firstName} ${updatingFlight.operator.lastName}</option>
                                                    </c:when>
                                                    <c:otherwise>
